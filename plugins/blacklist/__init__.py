...
delete_black_user = on_command('delete_black', permission=SUPERUSER)
...
async def _(bot, event, state):
    message = event.get_message()
    if len(message) == 0 or message[0].type != 'at':
        await add_black_user.finish('命令后面at要解除拉黑的人!')
        return

    # 获取拉黑人QQ
    qid = message[0].data['qq']
    if qid in bot.config.superusers:
        await add_black_user.finish('这事跟他没关系!')
        return

    # 查询是否存在
    result = await BlacklistModel.query_user(qid)
    if len(result) == 0:
        await add_black_user.finish('他并没有被拉黑')
        return

    await BlacklistModel.delete(qid)

    # 更新黑名单缓存
    blacklist_users = blacklist_cache.get('blacklist_users', default=[])
    blacklist_users.remove(qid)
    blacklist_cache.set('blacklist_users', blacklist_users)

    await add_black_user.finish(F"已取消拉黑")
